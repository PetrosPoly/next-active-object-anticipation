...
def query_and_log_llm(state, criteria, args, parameters, llama, current_time_s, project_path):
    ...
    """
    # 3-criteria gate
    if not (criteria.high_dot_counts_but_also_distance
    ...
    # the soon-to-approach object must have shown consistent focus history
    if not any(obj in criteria.high_dot_history for obj in criteria.less_than_2_seconds_dict.keys()):
        return None
