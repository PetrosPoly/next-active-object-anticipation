class LLMEvaluation:
    def __init__(self, llm_predictions, ground_truth):
        ...
        self.Tp_out = 0  # True Interaction
        self.Fp_out = 0  # False Interaction
        self.Fn = 0      # False Negatives
        self.Tp_in = 0   # Correct Object
        self.Fp_in = 0   # False Object
    ...
    def calculate_metrics(self):
            ...
            Correspondances gives us information about the
                1. LLM Activations
                2. Actual Interactions              (self.Tp + self.Fn) or total correspondances
                3. Missed Interactions              (self.Fn)
                4. Reduntant Interactions           (self.Fp_out)
                5. Correct object identification    (self.Tp_ob)
                3. Correct predictions              
            
            Based on these three, we calculate metrics to evaluate the performance of our algorithm on 2 Objectives

            A. Interaction Detection: Determine whether an interaction will occur at a certain point in time.
            B. Object Prediction: If an interaction occurs, assess how well the system’s top-3 object predictions match the actual object the user interacts with. 
            ...
            ******* Metrics *********
            ...
            A. 
                1. Accuracy: 
                        - Explanation: The proportion of all instances (interactions and non-interactions) that are correctly classified:
                        - Question:    Among all instances how many identied correcty as interactions
                        ...
                        - Up. Formula: Model_Overall_Accuracy = TP / TP + FP + FN 
                ...
                2. Precision (Positive_Prediction_Accuracy) : 
                        ...
                        - Question:    Of all the times your system predicted an interaction, how many were correct?
                        ...
                        - Formula:     Precision = TP / TP + FP_out 
                ...
                3. Recall (True_Positive_Rate): 
                        ...
                        - Question:    Of all the times an interaction actually occurred, how many did your system correctly predict?
                ...
                4. F1-score: 
                        - Explanation: The harmonic mean of Precision and Recall, providing a single measure of performance
                        - Formula:     2*(P*R)/(P+R)       

            B. (This part of the evaluation only applies to cases where an interaction actually took place)
                1. Top_3_Accuracy
                        - Explanation: This measures how often the correct object is found within the top-3 predicted objects
                        ...
                        - Formula:     TP_object = TP_object + FP_in
            """
            ...
            # Calculate Accuracy
            self.accuracy = round((self.Tp_out / (self.Tp_out + self.Fp_out + self.Fn)),3) if (self.Tp_out + self.Fp_out + self.Fn) else 0

            # Calculate Precision
            self.precision = round((self.Tp_out / (self.Tp_out + self.Fp_out)),3) if (self.Tp_out + self.Fp_out) else 0

            # Calculate Recall
            self.recall = round((self.Tp_out / (self.Tp_out + self.Fn)),3) if (self.Tp_out + self.Fn) else 0
            ...
            # top3_accuracy
            self.top3_accuracy = round((self.Tp_in / (self.Tp_in + self.Fp_in)), 3) if (self.Tp_in + self.Fp_in) else 0

            return (self.accuracy, self.precision, self.recall, self.top3_accuracy,
                   self.Tp_out, self.Fp_out, self.Fn, self.Tp_in, self.Fp_in,
                   self.total_ground_truth, self.total_llm_predictions, self.total_llm_activations, self.total_actual_correspondences, self.correspondences)   # self.correspondances have the in and the output while actual correspondance is calculated from self.correspondances and includes the actual correspondances
...
#             self.model_overall_accuracy = round((self.Tp / (self.Tp + self.Fp + self.Fn)),3) if (self.Tp + self.Fp + self.Fn) else 0

#             # Calculate Precision
#             self.precision = round((self.Tp / (self.Tp + self.Fp)),3) if (self.Tp + self.Fp) else 0

#             # NEW ONE - Precision Inside
#             self.precision_in = round((self.Tp / (self.Tp + self.Fp_in)), 3) if (self.Tp + self.Fp_in) else 0
...
#             self.recall = round((self.Tp / (self.Tp + self.Fn)),3) if (self.Tp + self.Fn) else 0
...
#             self.llm_activation_sensitivity = round((self.total_ground_truth / self.total_llm_predictions),3) if self.total_llm_predictions else 0
...
#             self.llm_interaction_accuracy = round((self.Tp / self.total_ground_truth),3) if self.total_ground_truth else 0
...
#                    self.correspondences, self.precision_in)  
