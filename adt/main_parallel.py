...
def run_simulation(parameters):
    ...
    def main():
        ...
        for timestamp_ns in tqdm(img_timestamps_ns):
            ...
            # VISIBLE DISTANCES
            visible_distance_camera_objects_scene = np.linalg.norm(visible_vector_camera_objects_scene, axis=1)     # Me: distance from camera to objects 
            visible_distance_device_objects_scene = np.linalg.norm(visible_vector_devive_objects_scene, axis=1)     # Me: distance from device to objects          
            distances =  visible_distance_device_objects_scene.tolist()                                             # Me: use the distance from the device to objects 
