...
from utils.rate_limit_v1 import AsyncRateLimiter, AsyncRateLimiter_per_second # this is a custom function of the rate limit 
...
async def activate_llm(log_content, parameters, paths, max_retries = 5):
    
    # Acquire rate limiter for each async call
    logging.info("Attempting to acquire rate limiter before LLM call.")
    await rate_limiter_per_sec.acquire() # Ensure rate limiting  
    ...
    prompts = read_prompts_from_file(paths['prompt'])
