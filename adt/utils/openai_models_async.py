...
from utils.rate_limit import rate_limiter, async_rate_limiter # this is a custom function of the rate limit 
...
async def activate_llm(log_content, parameters, max_retries = 5):
    
    await async_rate_limiter()  # Ensure rate limiting
...
#     try:
#         data = yaml.safe_load(cleaned_response)
#         most_likely_objects_to_interact_with = data.key['most_likely_objects_to_interact_with']
#         rationale = data.key['rationale']
#         predicted_interaction_objects = data.key['predicted_interaction_objects']
#         goal = data.key['goal_of_the_user']
#         return most_likely_objects_to_interact_with, rationale, predicted_interaction_objects, goal
#     except yaml.YAMLError as e:
#         logging.error(f"Error parsing YAML: {e}")
#         logging.error(f"Error parsing YAML: {e} | Parameters: {parameters}")
